...
namespace TP1
{
   public partial class Login : System.Web.UI.Page
   {
      ...
      protected void BTN_Login_Click(object sender, EventArgs e)
      {
         ...
         if (essaieDeConnexion <= 5 && !timeout)
         {
            if (TB_Username.Text == "")
            {
               LabelUsername.Text = "Nom d'usager requis.";
               connection = false;
            }
            else if (!utilisateur.Exist(TB_Username.Text))
            {
               LabelUsername.Text = "Le nom d'usager n'existe pas.";
               connection = false;
            }
            if (TB_Password.Text == "")
            {
               LabelPassword.Text = "Mot de passe requis.";
               connection = false;
            }
            ...
            if (connection)
            {
               ...
               {
                  LabelPassword.Text = "Mauvais mot de passe.";
               }
            }
         }
         else
         {
            ...
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Attention !", "alert('Vous avez un timeout !')", true);
         }
      }
      ...
      protected void BTN_PasswordReminder_Click(object sender, EventArgs e)
      {
         Users utilisateur = new Users((string)Application["MainDB"], this);
         if (TB_Username.Text == "")
            LabelUsername.Text = "Le nom d'usager est vide.";
         else if (!utilisateur.Exist(TB_Username.Text))
            LabelUsername.Text = "Le nom d'usager n'existe pas.";
         else
         {
